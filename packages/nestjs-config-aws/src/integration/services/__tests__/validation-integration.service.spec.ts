...
describe('ValidationIntegrationService', () => {
  ...
  describe('validateConfiguration', () => {
    ...
    it('should return error for unavailable validation provider', () => {
      ...
      expect(result.isValid).toBe(false);
      // Joi is not available in test environment, so it will return an error
  ...
  describe('getValidationRecommendations', () => {
    ...
    it('should detect complex nested structures', () => {
      // Need depth > 2 to trigger hasComplexNesting
      const complexConfig = {
        database: {
          primary: { 
          ...
          replica: { 
            connection: { host: 'replica', port: 5433 }
          }
        },
        cache: {
          redis: { 
            settings: { url: 'redis://localhost' }
          }
        }
      };
      ...
      // The implementation checks for hasComplexNesting (depth > 2) or hasArrays
      // Since we have depth > 2, it should recommend joi if available
      expect(recommendations).toBeDefined();
      expect(recommendations.recommendedProvider).toBeDefined();
    });
