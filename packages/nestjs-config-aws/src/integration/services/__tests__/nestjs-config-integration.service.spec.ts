...
describe('NestjsConfigIntegrationService', () => {
  ...
  describe('createConfigModuleOptions', () => {
    it('should create enhanced ConfigModule options with AWS factories', async () => {
      ...
      // Base options take precedence when explicitly set
      expect(enhancedOptions.isGlobal).toBe(false); // From base options (explicitly set)
      expect(enhancedOptions.cache).toBe(false); // From base options (explicitly set)
      expect(enhancedOptions.expandVariables).toBe(true); // From integration options (not set in base)
    ...
    it('should use base options when no integration options provided', async () => {
      ...
      expect(enhancedOptions.isGlobal).toBe(false); // From base options (explicitly set)
      expect(enhancedOptions.cache).toBe(false); // From base options (explicitly set)
      expect(enhancedOptions.expandVariables).toBe(false); // From base options (explicitly set)
    });
    ...
    it('should handle AWS factory registration failure with fallback', async () => {
      ...
      // Assert
      // When fallbackToLocal is true, it returns base options unchanged
      expect(enhancedOptions).toEqual(baseOptions);
    });
  ...
  describe('createAsyncConfigModuleOptions', () => {
    ...
    it('should handle async enhancement failure with fallback', async () => {
      ...
      // Assert
      // The method creates an enhanced useFactory and adds FactoryRegistrationService to inject
      expect(enhancedAsyncOptions.useFactory).toBeDefined();
      expect(enhancedAsyncOptions.inject).toContain(FactoryRegistrationService);
    });
  });
