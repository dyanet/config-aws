...
import { PrecedenceHandlerService } from '../precedence-handler.service';
...
describe('FactoryRegistrationService', () => {
  ...
  beforeEach(async () => {
    ...
    const mockPrecedenceHandler = {
      applyPrecedenceRules: jest.fn(),
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: PrecedenceHandlerService,
          useValue: mockPrecedenceHandler,
        },
        {
  ...
  describe('registerFactoriesAsync', () => {
    ...
    it('should throw error when AWS unavailable and failOnAwsError is true', async () => {
      // Arrange
      const failOnErrorOptions = { ...mockIntegrationOptions, failOnAwsError: true };
      ...
      // Act & Assert
      await expect(failService.registerFactoriesAsync()).rejects.toThrow('AWS services are not available');
    });
  });
